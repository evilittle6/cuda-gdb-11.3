...
static int cuda_read_memory_nonfocused(CORE_ADDR address, gdb_byte *buf, int len, struct type *type)
{
...
cuda_read_memory_partial (CORE_ADDR address, gdb_byte *buf, int len, struct type *type)
{
  ...
  if (!cuda_focus_is_device())
    {
      if (cuda_read_memory_nonfocused(address, buf, len, type) == 0)
	{
	  return 0;
	}
    }
      ...
      for (compunit_symtab *cu : objfile->compunits ())
        {
          for (symtab *s : compunit_filetabs (cu))
          {
            ...
            for (i = 0; i < BLOCKVECTOR_NBLOCKS (bv); i++)
            {
              ...
              lookup_name_info lookup_name (sym_name,
