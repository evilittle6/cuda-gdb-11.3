...
#ifdef CUDA_DEBUG_LINE_EXTENSION
#include "demangle.h"
#include "interps.h"
#endif
...
info_cuda_devices_command (const char *arg)
{
...

  {
	...
	uiout->field_int    ("device"         , d->device);
	...
	uiout->field_int    ("num_sms"        , d->num_sms);
	uiout->field_int	  ("num_warps"      , d->num_warps);
	uiout->field_int	  ("num_lanes"      , d->num_lanes);
	uiout->field_int	  ("num_regs"       , d->num_regs);
...
info_cuda_kernels_command (const char *arg)
{
...

  {
	...
	uiout->field_int       ("kernel"  , k->kernel_id);
	uiout->field_string    ("parent"  , k->parent);
	uiout->field_int       ("device"  , k->device);
  ...
  for (b = blocks, i = 0, kernel_id = ~0ULL; i < num_blocks; ++i, ++b)
    {
      ...
      if (uiout->is_mi_like_p ())
        uiout->field_int  ("kernel"  , b->kernel_id);
      uiout->field_string ("blockIdx", b->start_block_idx_string);
      uiout->field_string ("state"   , "running");
      uiout->field_int	  ("device"  , b->device);
      uiout->field_int	  ("sm"      , b->sm);
      uiout->text         ("\n");
    }
}
  ...
  for (b = threads, i = 0, kernel_id = ~0ULL; i < num_threads; ++i, ++b)
    {
      ...
      uiout->field_int    ("device"    , b->device);
      uiout->field_int    ("sm"        , b->sm);
      uiout->field_int    ("warp"      , b->wp);
      uiout->field_int    ("lane"      , b->ln);
      uiout->field_string ("filename"  , b->filename ? b->filename : "n/a");
      uiout->field_int    ("line"      , b->line);
      uiout->text         ("\n");
    }
}
  ...
  for (c = contexts, i = 0; i < num_contexts; ++i, ++c)
    {
      ...
      uiout->field_int    ("device"  , c->device);
  ...
  for (compunit_symtab *cu : objfile->compunits ())
    {
      for (symtab *s : compunit_filetabs (cu))
      {
        ...
        sym = block_lookup_symbol (block, MSYMBOL_PRINT_NAME(bmsym.minsym),
...
info_cuda_line_command (const char *args)
{
  ...
  if (filename)
      ...
      if (inline_info)
	{
	  ...
	  uiout->text         (":");
	  uiout->field_int    ("inline_line", inline_info->line);
	  uiout->text         (" by ");
